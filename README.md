...
|--|--|
| `200 OK` | Success. The body is a JSON array of snowflake IDs |
| `400 Bad Request` | The query string has an unknown field, a `count` that is not an integer, or a `count` that is not a positive integer within `MAX_BATCH_SIZE`. Also returned if the request has a body |
| `429 Too Many Requests` | The generation queue is full. The response includes a `Retry-After` header |
